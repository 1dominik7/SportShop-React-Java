...
import { ProductItemOneByColour } from "../types/userTypes";
...
const FavoritePage = () => {
    ...
    toggleFavorite.mutate(productItemId, {
      onSuccess: () => {
        setProductItems((prev) =>
          prev.filter((item) => item.id !== productItemId)
        );
      },
      onError: (error) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["userCart", auth.jwt] });
      setTimeout(() => {
                    ...
                    <div className="flex flex-col max-md:text-sm">
                      ...
                      {item?.variations.find(
                        ...
                        <span>
                          ...
                          <b>
                            {
                              item?.variations?.find((v) => v.name === "size")
                                ?.options[0].value
                            }
                          </b>
                        </span>
